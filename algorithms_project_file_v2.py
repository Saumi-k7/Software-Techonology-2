...
t1 = timeit.timeit(lambda: linear_search(insertion_sort(data.copy()), target), number=1)
print(f"Insertion Sort + Linear Search : {t1:.6f} sec")

# 2. Bubble Sort + Linear Search
t2 = timeit.timeit(lambda: linear_search(bubble_sort(data.copy()), target), number=1)
print(f"Bubble Sort    + Linear Search : {t2:.6f} sec")

# 3. Insertion Sort + Binary Search
t3 = timeit.timeit(lambda: binary_search(insertion_sort(data.copy()), target), number=1)
print(f"Insertion Sort + Binary Search : {t3:.6f} sec")

# 4. Bubble Sort + Binary Search
t4 = timeit.timeit(lambda: binary_search(bubble_sort(data.copy()), target), number=1)
print(f"Bubble Sort    + Binary Search : {t4:.6f} sec")

# Baseline: Unsorted Linear Search (no sorting needed)
t5 = timeit.timeit(lambda: linear_search(data, target), number=1)
print(f"Unsorted Linear Search (baseline): {t5:.6f} sec")